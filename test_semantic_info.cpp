...
void print_semantic_info(const nyxstone::Nyxstone::Instruction& insn)
{
    ...
    if (insn.semantic_info.has_value()) {
        ...
        std::cout << "    - opcode_name: " << sem.opcode_name << "\n";
        std::cout << "    - flags: 0x" << std::hex << sem.flags << std::dec << "\n";
        std::cout << "    - target_flags: 0x" << std::hex << sem.target_flags << std::dec << "\n";
        if (sem.is_branch)
            std::cout << "    - is_branch\n";
        if (sem.is_call)
            std::cout << "    - is_call\n";
        ...
        if (sem.is_conditional_branch)
            std::cout << "    - is_conditional_branch\n";
        if (sem.is_unconditional_branch)
            std::cout << "    - is_unconditional_branch\n";
        if (sem.is_indirect_branch)
            std::cout << "    - is_indirect_branch\n";
        if (sem.is_terminator)
            std::cout << "    - is_terminator\n";
        if (sem.is_barrier)
            std::cout << "    - is_barrier\n";
        if (sem.may_load)
            std::cout << "    - may_load\n";
        ...
        if (sem.has_unmodeled_side_effects)
            std::cout << "    - has_unmodeled_side_effects\n";
...
int main()
{
    ...
    // Test 8: CMP (check via opcode_name or flags)
